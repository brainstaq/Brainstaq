class Session::Voting
  ...
  include Brainstorm::States, Concepted, DoneVoting

  kredis_set :concept_votes,       typed: :integer, key: ->(v) { "user_concept_votes_#{v.session_id}_#{v.brainstorm.token}" }
  kredis_set :concept_build_votes, typed: :integer, key: ->(v) { "user_concept_build_votes_#{v.session_id}_#{v.brainstorm.token}" }
  ...
  def votes_cast_count
    concept_votes.size + concept_build_votes.size
  end
  ...
  def change_to_voting_done_state
    ...
    broadcast_concepts :transmit_concepts, transmit_concepts(sort_by_votes_desc)
  ...
  def votes_for(votable)
    votable.is_a?(Concept) ? concept_votes : concept_build_votes
  end
